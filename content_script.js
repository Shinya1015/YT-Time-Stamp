(function() {
    ...
    function loadState() {
        try {
            ...
            if (storedTimestamps) {
                ...
                catch (e) { console.error(LOG_TAG, 'タイムスタンプの読み込みに失敗:', e); timestamps = []; }
            } else { timestamps = []; }
            ...
            } else { console.warn(LOG_TAG, "loadState: bulkEditor or displayListElement not ready yet."); }
        } catch(e) {
        ...
        if (savedPosition) {
            try {
                ...
                if (pos && typeof pos === 'object' && typeof pos.left === 'string' && typeof pos.top === 'string') {
                    ...
                    console.log(LOG_TAG, "Loaded saved position:", pos);
                    return pos;
                } else { console.warn(LOG_TAG, "Saved position data invalid. Using default."); }
            } catch (e) { console.error(LOG_TAG, 'Failed to load position (JSON parse error?). Using default.', e); }
        } else { console.log(LOG_TAG, "No saved position. Using default."); }
        return defaultPos;
    }
    ...
    function addStyles() {
        ...
        const css = `
            ...
            #${TOGGLE_BUTTON_ID}.timestamp-ext-star-button-left {
                ...
                vertical-align: top; transition: opacity 0.1s linear, transform 0.1s ease-out;
                order: 5; cursor: pointer; background: none; border: none;
            }
    ...
    function runInitialization() {
        ...
        if (existingContainer && !isToggleButtonInjected) { console.log(LOG_TAG, "Container exists, ★ button missing. Injecting..."); container = existingContainer; checkAndInjectToggleButton(); if (initTimeoutId) clearTimeout(initTimeoutId); initTimeoutId = null; initRetryCount = 0; return; }
        else if (existingContainer && isToggleButtonInjected) { console.log(LOG_TAG, "Container & ★ button exist. Skipping."); if (initTimeoutId) clearTimeout(initTimeoutId); initTimeoutId = null; initRetryCount = 0; return; }
        if (initRetryCount >= MAX_INIT_RETRIES) { console.error(LOG_TAG, `Init timed out (${MAX_INIT_RETRIES} retries).`); if (initTimeoutId) clearTimeout(initTimeoutId); initTimeoutId = null; showErrorMessage("スクリプト初期化タイムアウト！"); initRetryCount = 0; return; }
        ...
        console.log(LOG_TAG, `[${initRetryCount}] Element check: video=${!!video}, videoReady=${videoReady}, player=${!!playerElement}`);
        if (videoReady && playerElement) {
            console.log(LOG_TAG, `[${initRetryCount}] Base elements ready. Initializing UI...`); if (initTimeoutId) clearTimeout(initTimeoutId); initTimeoutId = null; initRetryCount = 0;
            try { if (typeof initializeUI === 'function') { initializeUI(); if (!document.getElementById(CONTAINER_ID)) { console.error(LOG_TAG, "Panel container not found after init!"); showErrorMessage("UI追加失敗。"); } else { console.log(LOG_TAG, "Panel UI initialized. Starting ★ button check..."); checkAndInjectToggleButton(); } } else { console.error(LOG_TAG, "initializeUI missing!"); showErrorMessage("UI初期化関数が見つかりません！"); } }
            catch (e) { console.error(LOG_TAG, "Error during initializeUI call:", e); showErrorMessage("スクリプト初期化失敗！"); } return;
        }
        initRetryCount++; const retryDelay = INIT_RETRY_BASE_DELAY + initRetryCount * INIT_RETRY_INCREMENT; console.log(LOG_TAG, `[${initRetryCount-1}] Base elements not ready. Retrying in ${retryDelay}ms...`);
    ...
    function initialStart() {
       ...
       chrome.storage.sync.get([STORAGE_KEY_ENABLED], (result) => {
            isExtensionEnabled = (result[STORAGE_KEY_ENABLED] !== false); console.log(LOG_TAG, `Initial enabled state: ${isExtensionEnabled}`);
            if (isExtensionEnabled) {
                if (document.body) { if (location.href.includes('/watch?v=')) { console.log(LOG_TAG, "On watch page & enabled, calling runInitialization."); if (typeof runInitialization === 'function') { runInitialization(); } else { console.error(LOG_TAG, "initialStart: runInitialization missing!"); } } else { console.log(LOG_TAG, "Not on watch page, initial run skipped."); } }
                else { console.warn(LOG_TAG, "document.body not found, retrying initialStart."); setTimeout(initialStart, 100); }
            } else { console.log(LOG_TAG, "Extension initially disabled. Skipping."); cleanupExtensionUI(); }
       });
    }
    ...
    chrome.storage.onChanged.addListener((changes, namespace) => {
        if (namespace === 'sync' && changes[STORAGE_KEY_ENABLED]) {
            ...
            if (wasEnabled === nowEnabled) return; console.log(LOG_TAG, `Extension enabled state changed: ${wasEnabled} -> ${nowEnabled}`);
            isExtensionEnabled = nowEnabled;
            if (isExtensionEnabled) { console.log(LOG_TAG, "Extension enabled. Attempting UI init..."); if (location.href.includes('/watch?v=')) { runInitialization(); } else { console.log(LOG_TAG, "Enabled, but not on watch page. Skipping UI init."); } }
            else { console.log(LOG_TAG, "Extension disabled. Cleaning up UI..."); cleanupExtensionUI(); if (initTimeoutId) clearTimeout(initTimeoutId); initTimeoutId = null; initRetryCount = 0; if (playerControlsCheckInterval) clearInterval(playerControlsCheckInterval); playerControlsCheckInterval = null; }
        }
    });
