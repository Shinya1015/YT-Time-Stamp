(function() {
    ...
    const NUMBER_REGEX = /\[(\d+)\]/;           // Matches [NN] anywhere
    const GLOBAL_NUMBER_REGEX = /\[(\d+)\]/g;   // Finds ALL [NN] occurrences globally
    ...
    function loadState() {
        try {
            ...
            isLocked = localStorage.getItem('timestampLockState') === 'true'; firstTimeUser = localStorage.getItem('timestampFirstTime') === null; sortState = null;
            if (typeof applySavedPaneWidths === 'function') applySavedPaneWidths();
            // Initialize editor from loaded timestamps
            if (bulkEditor) {
                populateEditorFromTimestamps(); // Populate editor first
            }
            if (displayListElement && typeof renderTimestampList === 'function') {
                 renderTimestampList(); // Then render display list based on timestamps array
            }
            if (typeof applyLockState === 'function') applyLockState(); // Apply lock state after loading

        } catch(e) { timestamps = []; isLocked = false; firstTimeUser = true; sortState = null; }
    }
    ...
    function recordTimestamp() {
        ...
        try {
            ...
            console.error("Record Timestamp Error:", err);
        }
    }
        ...
        if (match) {
            try {
                ...
                if (lines.length > index) {
                    lines[index] = newTimestamp;
                    const cursorPos = bulkEditor.selectionStart;
                    bulkEditor.value = lines.join('\n');
                ...
                console.error("Adjust Timestamp Error:", e);
            }
    ...
    function deleteTimestamp(index) {
        ...
        try {
            ...
            if (lines.length >= index) {
                 ...
                 const cursorPos = bulkEditor.selectionStart;
                 bulkEditor.value = lines.join('\n');
                 const newPos = Math.min(cursorPos, bulkEditor.value.length);
                 bulkEditor.setSelectionRange(newPos, newPos);
            } else { populateEditorFromTimestamps(); }
            sortState = null;
            if(typeof updateSortButtonText === 'function') updateSortButtonText();
            if(typeof renderTimestampList === 'function') renderTimestampList();
            ...
            console.error("Delete Timestamp Error:", e);
        }
    }
    ...
    function jumpToTimestamp(timestamp) {
        ...
        if (match) {
            try {
                ...
                const video = document.querySelector('video');
                if (video) {
                    if (!isNaN(video.duration) && totalSeconds > video.duration) { video.currentTime = video.duration; }
                    else { video.currentTime = totalSeconds; }
                    if (typeof video.play === 'function') { video.play().catch(e => { console.warn("Video play interrupted:", e); }); }
    ...
    function deleteAllTimestampsConfirmed() {
        ...
        } catch (error) { showErrorMessage("全削除処理中にエラーが発生しました。"); console.error("Delete All Error:", error); }
    }
        ...
        editorChangeTimeout = setTimeout(() => {
            try {
                ...
                if (JSON.stringify(timestamps) !== JSON.stringify(lines)) {
                    ...
                    if (sortState !== null) { sortState = null; if(typeof updateSortButtonText === 'function') updateSortButtonText(); }
                    if(typeof renderTimestampList === 'function') renderTimestampList();
                }
    ...
    function renderTimestampList() {
        ...
        try {
            ...
            const currentTimestamps = timestamps; const validTimestampsForDisplay = currentTimestamps.map(String);
            let displayItems = []; let originalIndices = {};
            ...
            if (sortState !== null) {
                ...
                itemsToSort.sort((a, b) => {
                    const aIsEmptyOrInvalid = a.text.trim() === '' || a.timeSeconds === null;
                    const bIsEmptyOrInvalid = b.text.trim() === '' || b.timeSeconds === null;
                    if (aIsEmptyOrInvalid && bIsEmptyOrInvalid) { return a.originalIndex - b.originalIndex; }
                    else if (aIsEmptyOrInvalid) { return 1; }
                    else if (bIsEmptyOrInvalid) { return -1; }
            ...
            if (displayItems.length === 0 || (displayItems.length === 1 && displayItems[0].text === '')) {
                const emptyGuide = document.createElement('div'); emptyGuide.className = 'ts-empty-guide'; emptyGuide.textContent = "記録はありません"; displayListElement.appendChild(emptyGuide);
                 applyLockState(); return;
            }
            ...
            if (displayListElement) { displayListElement.textContent = ''; const errorLi = document.createElement('li'); errorLi.textContent = 'リスト表示エラー'; errorLi.style.cssText = 'color: red; padding: 10px; text-align: center;'; displayListElement.appendChild(errorLi); }
    ...
    function createTimestampListItem(timestampText, originalIndex) {
        try {
            ...
            const copyButton = document.createElement("button"); copyButton.textContent = "📋"; copyButton.className = "ts-copy-button ts-action-button"; copyButton.title = "この行をコピー"; copyButton.onclick = (e) => { e.stopPropagation(); copySingleTimestamp(textContent); }; actionButtons.push(copyButton);
            ...
            const deleteButton = document.createElement("button"); deleteButton.textContent = "削除"; deleteButton.className = "ts-delete-button ts-action-button"; deleteButton.onclick = (e) => { e.stopPropagation(); deleteTimestamp(originalIndex); }; actionButtons.push(deleteButton);
    ...
    function injectTimestampToggleButton() {
        ...
        // Using the original selectors for now, check console if they fail
        ...
        if (playPauseButton && leftControls) {
            ...
            if (buttonToInject) {
                try {
                    ...
                    console.log("Timestamp Record: Toggle button injected successfully.");
                    try {
                    ...
                    } catch (storageError) { console.warn("Failed to check/set star hint shown flag:", storageError); }
                    if (playerControlsCheckInterval) { clearInterval(playerControlsCheckInterval); playerControlsCheckInterval = null; }
                } catch (injectError) {
                    console.error("Failed to inject timestamp toggle button:", injectError);
                    removeTimestampToggleButton();
                }
            }
        } else {
             if (!playPauseButton) console.warn("Timestamp Record: Inject failed - '.ytp-play-button' not found.");
             if (!leftControls) console.warn("Timestamp Record: Inject failed - '.ytp-left-controls' not found.");
        }
    }
    ...
    function initializeUI() {
        ...
        } catch (uiError) { // This catch corresponds to the main try block in initializeUI
            showErrorMessage("スクリプトUIの読み込みに失敗しました！");
            if (container?.parentNode) { try { container.remove(); } catch(e) {} }
            container = null;
            if (containerResizeObserver) { try { containerResizeObserver.disconnect(); } catch(e) {} containerResizeObserver = null; }
            console.error("InitializeUI Error:", uiError);
        }
    }
    ...
    function runInitialization() {
        ...
        if (existingContainer) {
            if (!isToggleButtonInjected) { container = existingContainer; checkAndInjectToggleButton(); }
            if (initTimeoutId) clearTimeout(initTimeoutId); initTimeoutId = null; initRetryCount = 0;
            return;
        }
        if (initRetryCount >= MAX_INIT_RETRIES) {
            if (initTimeoutId) clearTimeout(initTimeoutId); initTimeoutId = null;
            showErrorMessage("スクリプト初期化タイムアウト！"); initRetryCount = 0;
            return;
        }
        const video = document.querySelector('video'); const playerElement = document.getElementById('movie_player');
        const videoReady = video && typeof video.currentTime === 'number' && video.readyState >= 1;
        if (videoReady && playerElement) {
            if (initTimeoutId) clearTimeout(initTimeoutId); initTimeoutId = null; initRetryCount = 0;
            try { // <-- Start of the try block inside runInitialization
                if (typeof initializeUI === 'function') {
                    initializeUI();
                    if (!document.getElementById(CONTAINER_ID)) { showErrorMessage("UI追加失敗。"); }
                    else { checkAndInjectToggleButton(); }
                } else { showErrorMessage("UI初期化関数が見つかりません！"); }
            } catch (e) { // <--- *** ADDED MISSING CATCH BLOCK HERE ***
                showErrorMessage("スクリプト初期化失敗！");
                console.error("Run Initialization Error during UI init:", e);
            } // <--- *** END OF ADDED CATCH BLOCK ***
            return; // This return was already here, after the (previously missing) catch
        }
        initRetryCount++;
    ...
    try { // <-- Start of the try block for MutationObserver setup
        ...
        } else { console.warn("Timestamp Record: Could not find body to observe SPA navigation."); }
    } catch (e) { // <--- *** ADDED MISSING CATCH BLOCK HERE ***
        console.error("Timestamp Record: Failed to set up MutationObserver.", e);
    } // <--- *** END OF ADDED CATCH BLOCK ***
