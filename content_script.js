(function() {
    ...
    function loadState() {
        try {
            ...
            if (displayListElement && typeof renderTimestampList === 'function') { renderTimestampList(); }
    ...
    function recordTimestamp() {
        ...
        try {
            ...
            if (video && typeof video.currentTime === 'number' && !isNaN(video.currentTime)) {
                ...
                // *** Store scroll position BEFORE changing value ***
                const editorScrollTop = bulkEditor.scrollTop;
                const editorScrollLeft = bulkEditor.scrollLeft;
                let listScrollTop = 0;
                if (displayListElement) {
                    listScrollTop = displayListElement.scrollTop;
                }
                ...
                bulkEditor.scrollTop = editorScrollTop;
                bulkEditor.scrollLeft = editorScrollLeft;

                if(typeof renderTimestampList === 'function') {
                    renderTimestampList(); // Render list based on current sortState
                    // Restore list scroll position after render
                    if (displayListElement) {
                         requestAnimationFrame(() => { // Use rAF for timing
                             displayListElement.scrollTop = listScrollTop;
                         });
                    }
                }
    ...
    // --- vvv MODIFIED adjustTimestamp (Preserve Scroll) vvv ---
    function adjustTimestamp(index, adjustment) {
        if (index < 0 || index >= timestamps.length) return;
        if (isLocked || !bulkEditor || !displayListElement) return; // Need displayListElement
        ...
        if (match) {
            ...
            const listScrollTop = displayListElement.scrollTop;
            const editorScrollTop = bulkEditor.scrollTop; // Also save editor scroll
            ...
            try {
                ...
                // Update editor value and maintain cursor/scroll
                ...
                if (lines.length > index) {
                    ...
                    // Restore cursor and editor scroll AFTER updating value
                    bulkEditor.setSelectionRange(cursorPos, cursorPos);
                    bulkEditor.scrollTop = editorScrollTop;
                } else {
                    populateEditorFromTimestamps(); // Fallback
                    bulkEditor.scrollTop = editorScrollTop; // Try to restore scroll even on fallback
                }

                // No longer resetting sort state here to maintain view
                ...
                // Jump and show message (no changes needed here)
                ...
                showErrorMessage("時間調整エラー。"); console.error("Adjust Timestamp Error:", e);
                // Restore scroll even if error occurs
                requestAnimationFrame(() => {
                     displayListElement.scrollTop = listScrollTop;
                     bulkEditor.scrollTop = editorScrollTop;
                });
    ...
    // --- ^^^ MODIFIED adjustTimestamp ^^^ ---

    // --- vvv MODIFIED deleteTimestamp (Preserve Scroll) vvv ---
    function deleteTimestamp(index) {
        if (index < 0 || index >= timestamps.length) return;
        if (isLocked || !displayListElement) return; // Need displayListElement
        ...
        try {
            ...
            // Update editor and maintain scroll/cursor
            ...
            if (index >= 0 && index < lines.length) {
                 ...
                 bulkEditor.scrollTop = editorScrollTop; // Restore editor scroll
            } else {
                 ...
                 bulkEditor.scrollTop = editorScrollTop; // Try to restore scroll on fallback
            }
            ...
            showErrorMessage("削除エラー。"); console.error("Delete Timestamp Error:", e);
            // Restore scroll even if error occurs
             requestAnimationFrame(() => {
                 displayListElement.scrollTop = listScrollTop;
                 bulkEditor.scrollTop = editorScrollTop;
    ...
    // Keep modified handleEditorChange (maintains sort order on edit)
        ...
        editorChangeTimeout = setTimeout(() => {
            try {
                ...
                if (JSON.stringify(timestamps) !== JSON.stringify(lines)) {
                    ...
                    // Render using the CURRENT sortState
        ...
        try {
            ...
            adjustmentSelect.addEventListener('change', (event) => {
                ...
                if (!isNaN(newStep) && ALLOWED_ADJUSTMENT_STEPS.includes(newStep)) {
                    ...
                    // Directly update button text without full re-render if possible
                    const adjustButtons = displayListElement?.querySelectorAll('.ts-adjust-button');
                    adjustButtons?.forEach(btn => {
                         if(btn.classList.contains('ts-minus-button')) { btn.textContent = `-${currentAdjustmentStep}s`; btn.title = `-${currentAdjustmentStep}秒`; }
                         if(btn.classList.contains('ts-plus-button')) { btn.textContent = `+${currentAdjustmentStep}s`; btn.title = `+${currentAdjustmentStep}秒`; }
                    });
                    if (typeof applyLockState === 'function') applyLockState();
