(function() {
    ...
    // --- vvv MODIFIED recordTimestamp (Removed sortState reset) vvv ---
    function recordTimestamp() {
        ...
        try {
            ...
            if (video && typeof video.currentTime === 'number' && !isNaN(video.currentTime)) {
                ...
                // *** REMOVED: Resetting sortState ***
                // if (sortState !== null) { sortState = null; updateSortButtonText(false); }

                // Re-render the list using the CURRENT sortState
    ...
    function adjustTimestamp(index, adjustment) {
        ...
        if (match) {
            try {
                ...
                // Keep sortState reset for adjustment, as it modifies existing data order
                if (sortState !== null) { sortState = null; updateSortButtonText(false); }

                if(typeof renderTimestampList === 'function') renderTimestampList();
                if(typeof jumpToTimestamp === 'function') { jumpToTimestamp(newTimestamp); }
                const timePart = newTimestamp.match(TIME_REGEX)?.[0] || newTimestamp;
                showAdjustmentSuccessMessage(adjustment, timePart);
    ...
    function deleteTimestamp(index) {
        ...
        if (isLocked) return;
        try {
            const deletedContent = String(timestamps[index]);
            const previewLength = 25;
            const deletedContentPreview = deletedContent.length > previewLength ? deletedContent.substring(0, previewLength) + '...' : deletedContent;
            ...
            if (index >= 0 && index < lines.length) {
                ...
                bulkEditor.value = lines.join('\n');
                const newPos = Math.min(currentCursorPos, bulkEditor.value.length);
                try { bulkEditor.setSelectionRange(newPos, newPos); } catch (selectionError) { console.warn("Failed to set selection range after delete:", selectionError); }
            } else {
                console.warn(`deleteTimestamp: Index ${index} invalid for editor lines (${lines.length}). Repopulating editor.`);
                populateEditorFromTimestamps();
                try { bulkEditor.setSelectionRange(bulkEditor.value.length, bulkEditor.value.length); } catch (selectionError) { console.warn("Failed to set selection range after repopulate:", selectionError); }
            }

            // Keep sortState reset for deletion
            if (sortState !== null) { sortState = null; updateSortButtonText(false); }
            ...
            try {
                ...
                if (video) {
                    ...
                    if (typeof video.play === 'function') {
                        ...
                        if (playPromise !== undefined) { playPromise.catch(error => { if (error.name !== 'AbortError') { console.warn("Timestamp Record: Video play() failed:", error); } }); }
                    }
    ...
    function toggleSortOrder() {
        if (isLocked) return;
        if (sortState === null) { sortState = true; } else if (sortState === true) { sortState = false; } else { sortState = null; }
        if(typeof renderTimestampList === 'function') renderTimestampList();
        updateSortButtonText(true);
    }

    function updateSortButtonText(animate = true) {
        try {
            const btn = document.getElementById('ts-sort-button');
            if (!btn) return;
            if (sortState === true) { btn.textContent = "時間昇順 ▲"; } else if (sortState === false) { btn.textContent = "時間降順 ▼"; } else { btn.textContent = "元の順序"; }
            if (animate) {
                btn.style.transition = 'transform 0.1s ease-out'; btn.style.transform = "scale(0.95)";
                setTimeout(() => { if(btn) btn.style.transform = "scale(1)"; }, 100);
            } else { btn.style.transition = 'none'; btn.style.transform = ""; }
        } catch(e) { console.error("Update Sort Button Text Error:", e); }
    }
    ...
    function deleteAllTimestampsConfirmed() {
        ...
        try {
            ...
            sortState = null; updateSortButtonText(false);
            if(typeof renderTimestampList === 'function') renderTimestampList();
    ...
    function applyLockState() {
        if (!lockButton || !container || !bulkEditor || !topBarElement || !bottomBarElement || !resizerElement || !recordBtn) { return; }
        try {
            lockButton.textContent = isLocked ? "アンロック" : "ロック"; lockButton.classList.toggle('ts-locked', isLocked); lockButton.classList.toggle('ts-unlocked', !isLocked);
            bulkEditor.readOnly = isLocked; bulkEditor.style.backgroundColor = isLocked ? '#eee' : '#fff'; bulkEditor.style.cursor = isLocked ? 'not-allowed' : '';
            topBarElement.style.cursor = isLocked ? 'default' : 'move'; bottomBarElement.style.cursor = isLocked ? 'default' : 'move'; topBarElement.classList.toggle('ts-locked', isLocked); bottomBarElement.classList.toggle('ts-locked', isLocked);
            recordBtn.disabled = isLocked; recordBtn.style.opacity = isLocked ? '0.6' : '1'; recordBtn.style.cursor = isLocked ? 'not-allowed' : 'pointer';
            if (!container.dataset?.originalResize) { container.dataset.originalResize = window.getComputedStyle(container).resize || 'both'; } container.style.resize = isLocked ? 'none' : (container.dataset.originalResize); container.classList.toggle('ts-locked', isLocked);
            resizerElement.style.display = isLocked ? 'none' : 'block'; resizerElement.style.cursor = isLocked ? 'default' : 'col-resize';
            const actionButtons = displayListElement?.querySelectorAll('.ts-action-button'); actionButtons?.forEach(btn => { btn.disabled = isLocked; btn.style.opacity = isLocked ? 0.5 : 1; btn.style.cursor = isLocked ? 'not-allowed' : 'pointer'; });
            const listControls = displayPane?.querySelectorAll('.ts-list-button, .ts-adjustment-select'); listControls?.forEach(ctrl => { ctrl.disabled = isLocked; ctrl.style.opacity = isLocked ? 0.6 : 1; ctrl.style.cursor = isLocked ? 'not-allowed' : 'pointer'; if (ctrl.tagName === 'SELECT') { ctrl.style.backgroundColor = isLocked ? '#eee' : ''; ctrl.style.borderColor = isLocked ? '#ccc' : ''; } });
        } catch(e) { console.error("Apply Lock State Error:", e); }
    }
        ...
        editorChangeTimeout = setTimeout(() => {
            try {
                ...
                if (JSON.stringify(timestamps) !== JSON.stringify(lines)) {
                    ...
                    // Keep sortState reset for manual editor changes
                    if (sortState !== null) { sortState = null; updateSortButtonText(false); }
                    if(typeof renderTimestampList === 'function') renderTimestampList();
                }
        ...
        try {
            ...
            if (sortState !== null) {
                ...
                itemsToSort.sort((a, b) => {
                    const aIsEmptyOrInvalid = a.text.trim() === '' || a.timeSeconds === null; const bIsEmptyOrInvalid = b.text.trim() === '' || b.timeSeconds === null;
                    if (aIsEmptyOrInvalid && bIsEmptyOrInvalid) { return a.originalIndex - b.originalIndex; } else if (aIsEmptyOrInvalid) { return 1; } else if (bIsEmptyOrInvalid) { return -1; }
    ...
    function createTimestampListItem(timestampText, originalIndex) {
        try {
            const textContent = String(timestampText); const listItem = document.createElement("li"); listItem.className = "ts-list-item"; listItem.dataset.originalIndex = originalIndex;
            const itemContainer = document.createElement("div"); itemContainer.className = "ts-item-container"; const hasValidTime = TIME_REGEX.test(textContent); const actionButtons = [];
            ...
            if (hasValidTime) {
                const jumpIcon = document.createElement("span"); jumpIcon.textContent = "▶️"; jumpIcon.className = "ts-jump-icon"; jumpIcon.title = "クリックでジャンプ"; jumpIcon.onclick = (e) => { e.stopPropagation(); jumpToTimestamp(textContent); }; actionButtons.push(jumpIcon);
    ...
    function injectTimestampToggleButton() {
        ...
        if (playPauseButton && leftControls) {
            ...
            if (buttonToInject) {
                try {
                    playPauseButton.insertAdjacentElement('afterend', buttonToInject); isToggleButtonInjected = true; console.log("Timestamp Record: Toggle button injected successfully.");
                    try { const hintShown = localStorage.getItem(STORAGE_KEY_STAR_HINT_SHOWN); if (!hintShown) { showInfoMessage("ヒント：プレーヤー左下の ★ ボタンでパネルを表示/非表示できます。", 6000); localStorage.setItem(STORAGE_KEY_STAR_HINT_SHOWN, 'true'); } } catch (storageError) { console.warn("Failed to check/set star hint shown flag:", storageError); }
                    if (playerControlsCheckInterval) { clearInterval(playerControlsCheckInterval); playerControlsCheckInterval = null; }
                } catch (injectError) { console.error("Failed to inject timestamp toggle button:", injectError); removeTimestampToggleButton(); }
            }
        } else { if (!playPauseButton) console.warn("Timestamp Record: Inject failed - '.ytp-play-button' not found."); if (!leftControls) console.warn("Timestamp Record: Inject failed - '.ytp-left-controls' not found."); }
    }
    ...
    function runInitialization() {
        ...
        if (existingContainer) { if (!isToggleButtonInjected) { container = existingContainer; checkAndInjectToggleButton(); } if (initTimeoutId) clearTimeout(initTimeoutId); initTimeoutId = null; initRetryCount = 0; return; }
        ...
        if (videoReady && playerElement) {
            ...
            catch (e) { showErrorMessage("スクリプト初期化失敗！"); console.error("Run Initialization Error during UI init:", e); }
            return;
        }
        initRetryCount++; const retryDelay = INIT_RETRY_BASE_DELAY + initRetryCount * INIT_RETRY_INCREMENT; if (initTimeoutId) clearTimeout(initTimeoutId); initTimeoutId = setTimeout(runInitialization, retryDelay);
    }
